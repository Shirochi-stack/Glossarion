...
from PySide6.QtWidgets import (
    ...
    QProgressBar, QApplication, QMenu, QSlider, QButtonGroup
)
from PySide6.QtCore import Qt, QSize, Signal, QThread, QTimer
from PySide6.QtGui import QPixmap, QFont, QIcon, QImage, QAction, QCursor
...
def _find_halgakos_icon() -> str | None:
    ...
    # Also try next to the frozen executable
...
class _BookCard(QFrame):
    ...
    clicked = Signal(dict)
    context_menu_requested = Signal(dict, object)  # (book, QPoint)
    ...
    def __init__(self, book: dict, preset: dict | None = None, parent=None):
        ...
        self.cover_label.setStyleSheet(
            "background: #2a2a3e; border-radius: 4px; color: #555; font-size: 28pt;"
        )
        # Try Halgakos fallback immediately
    ...
    def _set_fallback_icon(self, icon_path: str):
        """Set a scaled-down Halgakos icon as the cover placeholder."""
        try:
            pm = QPixmap(icon_path)
            if not pm.isNull():
                # Scale to fit inside cover area at ~60% for a tasteful placeholder
...
class EpubLibraryDialog(QDialog):
    """Grid-based EPUB library browser with sorting, size presets, and context menus."""
    ...
    def _make_sort_btn(self, text: str, tooltip: str, mode: str) -> QPushButton:
    ...
    def _make_size_btn(self, text: str, tooltip: str, size_key: str) -> QPushButton:
        btn = QPushButton(text)
        btn.setToolTip(tooltip)
    ...
    def _setup_ui(self):
        ...
        banner_lbl.setStyleSheet("color: #8ad; font-size: 8.5pt;")
    ...
    def _set_card_size(self, size_key: str):
        self._card_size = size_key
        for k, btn in self._size_btns.items():
            btn.setChecked(k == size_key)
        self._refresh_view()

    def _sorted_books(self, books: list[dict]) -> list[dict]:
    ...
    def _populate_grid(self, books: list[dict]):
        ...
        self._relocate_banner.setVisible(bool(outside))
    ...
    def _show_context_menu(self, book: dict, pos):
        menu = QMenu(self)
        menu.setStyleSheet("""
            QMenu {
                background: #1e1e2e; border: 1px solid #3a3a5e; border-radius: 4px;
        ...
        copy_path_action.triggered.connect(
            lambda: QApplication.clipboard().setText(book["path"])
        )
...
class EpubReaderDialog(QDialog):
    ...
    def _apply_reader_style(self):
        if self._dark_mode:
            bg, fg, link = "#1a1a2e", "#d0d0e0", "#6c9bd2"
        else:
            bg, fg, link = "#fafafa", "#222222", "#1a73e8"

        self._reader.setStyleSheet(f"""
            QTextBrowser {{
                background: {bg}; color: {fg}; border: none;
                padding: 20px 30px; font-size: {self._font_size}pt; line-height: 1.7;
            }}
