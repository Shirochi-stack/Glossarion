...
class UnifiedClient:
    ...
    def _send_openrouter(self, messages, temperature, max_tokens, response_name) -> UnifiedResponse:
        """Send request to OpenRouter API - avoids shared-state mutation"""
        # Check if safety settings are disabled via GUI toggle
        disable_safety = os.getenv("DISABLE_GEMINI_SAFETY", "false").lower() == "true"
        
        # Determine model to send (strip 'or/' or 'openrouter/' prefix only for transport)
        # Read current model under microsecond lock to avoid cross-thread contamination
        if hasattr(self, '_instance_model_lock'):
            with self._instance_model_lock:
                model_to_send = self.model
        else:
            model_to_send = self.model
        for prefix in ['or/', 'openrouter/']:
            if model_to_send.startswith(prefix):
                model_to_send = model_to_send[len(prefix):]
                break
        
        # Normalize whitespace just in case
        model_to_send = model_to_send.strip()
        ...
        # Perform API call without mutating instance model
        return self._send_openai_compatible(
            messages, temperature, max_tokens,
        ...
        if openai and provider in sdk_compatible:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    # Use pooled session and idempotency key
                    ...
                    if resp.status_code == 429:
                        # Extract any retry information if available and honor it briefly
                        ...
                        if retry_secs:
                            ...
                            print(f"{provider} 429 Retry-After={retry_secs}s; sleeping {sleep_for:.1f}s before raising")
                            waited = 0.0
                    ...
                    if resp.status_code != 200:
                        ...
                        error_msg = f"{provider} API error: {status} - {resp.text}"
                        # Honor Retry-After for common transient errors
                        if status in (500, 502, 503, 504):
                            retry_after_val = resp.headers.get('Retry-After', '')
                            retry_secs = self._parse_retry_after(retry_after_val)
                            if retry_secs:
                                max_cap = int(os.getenv("RETRY_AFTER_MAX_WAIT", "120"))
                                sleep_for = min(retry_secs, max_cap) + random.uniform(0, 1)
                                print(f"{provider} {status} Retry-After={retry_secs}s; sleeping {sleep_for:.1f}s")
                                waited = 0.0
                                while waited < sleep_for:
                                    if self._cancelled:
                                        raise UnifiedClientError("Operation cancelled by user", error_type="cancelled")
                                ...
                                print(f"Transient server error {status} - retrying in {delay:.1f}s (attempt {attempt + 1}/{max_retries})")
                                waited = 0.0
                                while waited < delay:
                                    if self._cancelled:
                                        raise UnifiedClientError("Operation cancelled by user", error_type="cancelled")
                        ...
                        if attempt < max_retries - 1:
                            ...
                            time.sleep(api_delay)
                            continue
                        raise UnifiedClientError(error_msg, http_status=status)
