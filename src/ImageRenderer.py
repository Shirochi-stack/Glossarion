...
def _render_with_manga_translator(self, image_path: str, regions, output_path: str = None, image_bgr=None, original_image_path: str = None, switch_tab: bool = True):
    ...
    try:
        ...
        # ⚡ CACHE COMPOSITOR: Store base image + individual region overlays for instant updates
        try:
            if not hasattr(self, 'fast_compositor'):
                from fast_overlay_compositor import FastOverlayCompositor
                self.fast_compositor = FastOverlayCompositor()
            
            # Set base cleaned image
            self.fast_compositor.set_base_image(image_bgr.copy())
            
            # Cache individual region overlays by re-rendering each one separately
            print(f"⚡ Caching {len(filtered_regions)} individual region overlays...")
            ...
            for idx, region in enumerate(filtered_regions):
                try:
                    ...
                    img_size = (image_bgr.shape[1], image_bgr.shape[0])  # (width, height)
                    overlay = PIL_Image.new('RGBA', img_size, (0, 0, 0, 0))
                    draw = ImageDraw.Draw(overlay)
                    ...
                    if text:  # Only render if there's text
                        # Use translator settings for consistent rendering
                        try:
                            font_size = int(self._manga_translator.custom_font_size or 24)
                            font_path = self._manga_translator.selected_font_style
                            if font_path and os.path.exists(font_path):
                                font = ImageFont.truetype(font_path, font_size)
                            else:
                                font = ImageFont.load_default()
                            
                            text_color = tuple(self._manga_translator.text_color) + (255,)
                            outline_color = tuple(self._manga_translator.outline_color) + (255,)
                            outline_width = max(1, font_size // self._manga_translator.outline_width_factor)
                            ...
                            for i, line in enumerate(lines):
                                ...
                                # Clamp to bounds
                                tx = max(0, min(tx, img_size[0] - 10))
                                ty = max(0, min(ty, img_size[1] - 10))
                            ...
                            print(f"⚡ Render error for region {idx}: {render_err}")
                    
                    self.fast_compositor.cache_region_overlay(idx, overlay, position)
                except Exception as cache_err:
                    print(f"⚡ Failed to cache overlay {idx}: {cache_err}")
            
            self._compositor_initialized = True
            print(f"⚡ Fast compositor: base + {len(filtered_regions)} region positions cached")
