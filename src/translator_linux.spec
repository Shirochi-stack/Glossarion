...
"""
Glossarion - PyInstaller Specification File (Linux)
Enhanced Translation Tool with QA Scanner, and AI Hunter
Adapted from translator.spec with the same Standard feature selections.
"""
...
APP_NAME = get_spec_app_name("translator_linux.spec")
...
# GUI Framework
gui_modules = [
    # Explicit imports bundle Qt WebEngine libraries, helper process and resources.
...
# Build on Linux from src/: pyinstaller translator_linux.spec --noconfirm
# Install requirements.txt and the Linux system dependencies from build-linux.yml.
