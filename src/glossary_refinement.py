...
- For character entries, first check whether a Surname or Surnames entry type exists. If it exists and the provided glossary clearly identifies a character's surname, combine that surname with the character's given name into ONE full-name character entry. For example, if John is identified as a member of the Smith family, the character entry must be John Smith, not just John. Do NOT split this full name into separate character entries. ONLY separate a full name into given-name and surname entries when there is NO Surname/Surnames entry type. If a character's surname is unknown or the connection is unclear, keep the character's name unchanged; never guess a surname. Do not add titles, nicknames, or aliases to the full name. Use the source-language full name in raw_name and its target-language form in translated_name, preserving the appropriate name order in each language.
