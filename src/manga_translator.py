...
# Build a region overlay in a separate process. Returns a small RGBA tile and where to place it.
# Spec keys expected:
#  - idx, canvas_size (w,h), rect (x,y,w,h), text, force_caps, font_path, font_size_mode,
#    custom_font_size, text_color, outline_color, outline_width_factor,
#    shadow_enabled, shadow_color, shadow_offset (dx,dy),
#    bg_style, bg_opacity, bg_reduction, draw_bg (bool)
from typing import Any as _Any

def _render_overlay_worker(spec: dict) -> tuple[int, int, int, int, int, bytes] | None:
    try:
        ...
        draw_bg = bool(spec.get('draw_bg', True))

        # Create local ROI overlay equal to rect size
        ...
        # Word wrap by width using textbbox
        ...
        # Fit font size to ROI
        def _fit(text: str, max_w: int, max_h: int):
            if font_size_mode == 'fixed' and custom_font_size > 0:
                f = _get_font(custom_font_size)
                lines = _wrap(text, f, max_w)
                # compute height
                sample = lines[0] if lines else 'Ay'
                bbox = draw.textbbox((0, 0), sample, font=f)
                lh = max(int(custom_font_size * 1.2), int((bbox[3] - bbox[1]) * 1.1))
                total_h = lh * max(1, len(lines))
                # If it doesn't fit, downscale
                sz = custom_font_size
                while total_h > max_h or any((draw.textbbox((0, 0), ln, font=f)[2] - draw.textbbox((0, 0), ln, font=f)[0]) > max_w for ln in lines):
                    sz = max(8, sz - 1)
                    f = _get_font(sz)
                    lines = _wrap(text, f, max_w)
        ...
        # Background
        if draw_bg and bg_opacity > 0:
            padding = int(font_size * 0.3)
            # compute max width of lines
            max_width = 0
            for ln in lines:
                bbox = draw.textbbox((0, 0), ln, font=font)
                max_width = max(max_width, bbox[2] - bbox[0])
            bg_w = int((max_width + padding * 2) * float(bg_reduction))
            bg_h = int(((len(lines) * line_height) + padding * 2) * float(bg_reduction))
            bg_x = (roi_w - bg_w) // 2
            ...
            if bg_style == 'box':
                ...
                for i, ln in enumerate(lines):
                    ...
                    line_bg_w = int((line_width + padding) * float(bg_reduction))
        ...
        # Text
        # Compute start_y so vertical fits
...
class MangaTranslator:
            ...
            with MangaTranslator._inpaint_pool_lock:
                ...
                if rec and 'checked_out' in rec:
                    ...
                    if self._checked_out_inpainter in checked_out:
                        # Do NOT touch global CUDA state here; just clear per-instance temp refs
                        try:
    ...
    def __init__(self, ocr_config: dict, unified_client, main_gui, log_callback=None):
        ...
        # Cached variables have been removed - see properties below in this class
                    ...
                    if rtdetr_detections:
                        ...
                        if not all_regions:
                            ...
                            if full_image_ocr:
                                ...
                                debug_matching = self.main_gui.config.get('manga_settings', {}).get('advanced', {}).get('debug_mode', False)
                ...
                if self.ocr_provider == 'manga-ocr':
                    ...
                    if ocr_settings.get('bubble_detection_enabled', False):
                        ...
                        if rtdetr_detections:
                            ...
                            if self.main_gui.config.get('manga_settings', {}).get('advanced', {}).get('parallel_processing', True) and len(all_regions) > 1:
                        ...
                        if rtdetr_detections:
                            ...
                            if not all_regions:
                                ...
                                if full_image_ocr:
                                    ...
                                    self._log(f"🔗 Step 2: Matching {len(full_image_ocr)} OCR lines to {len(all_regions)} RT-DETR blocks")
                                    source_lang = ocr_settings.get('language_hints', ['ja'])[0] if ocr_settings.get('language_hints') else 'ja'
                                    matched_blocks = match_ocr_to_rtdetr_blocks(full_image_ocr, all_regions, source_lang, debug=self.main_gui.config.get('manga_settings', {}).get('advanced', {}).get('debug_mode', False))
    ...
    def create_text_mask(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        if self.main_gui.config.get('manga_settings', {}).get('advanced', {}).get('debug_mode', False):
    ...
    def render_translated_text(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        if use_rgba_rendering:
            ...
            try:
                ...
                max_workers = 4
            # Select render parallel mode (env override > config > default 'process')
            try:
                mode_env = os.environ.get('RENDER_PARALLEL_MODE', '').strip().lower()
            except Exception:
                mode_env = ''
            mode_cfg = ''
            try:
            ...
            if render_parallel and len(adjusted_regions) > 1:
                # Choose parallel mode: process (default) or thread
                mode = parallel_mode
                # Clamp workers
                workers = max(1, min(max_workers, len(adjusted_regions)))
                self._log(f"⚡ Render parallel mode: {mode} (workers={workers})", "info")
                if mode == 'process':
                    # Build specs for each region to render in subprocess (smaller ROI tiles)
                    specs = []
                    for i, r in enumerate(adjusted_regions):
                        if not r.translated_text:
                            specs.append(None)
                            continue
                        # Derive render rect like in _render_one
                        x, y, w, h = r.bounding_box
                        if hasattr(self, 'safe_area_enabled') and not self.safe_area_enabled:
                            rx, ry, rw, rh = x, y, w, h
                        elif use_mask_for_rendering and text_mask is not None:
                            sx, sy, sw, sh = self.get_safe_text_area(r, use_mask_bounds=True, full_mask=text_mask)
                            rx, ry, rw, rh = sx, sy, sw, sh
                        elif hasattr(r, 'vertices') and r.vertices:
                            if hasattr(self, 'safe_area_enabled') and not self.safe_area_enabled:
                                rx, ry, rw, rh = x, y, w, h
                            else:
                                sx, sy, sw, sh = self.get_safe_text_area(r, use_mask_bounds=False)
                                rx, ry, rw, rh = sx, sy, sw, sh
                        else:
                            rx, ry, rw, rh = x, y, w, h
                        # Determine if bg should draw only for free-text
                        draw_bg = True
                        try:
                            if getattr(self, 'free_text_only_bg_opacity', False):
                                draw_bg = self._is_free_text_region(r)
                        except Exception:
                            pass
                        specs.append({
                            'idx': i,
                            ...
                            'text': (r.translated_text or '').strip(),
                            'force_caps': bool(self.force_caps_lock),
                            'font_path': self.selected_font_style or self.font_path,
                            'font_size_mode': self.font_size_mode if hasattr(self, 'font_size_mode') else 'multiplier',
                            'custom_font_size': int(getattr(self, 'custom_font_size', 0) or 0),
                            'text_color': tuple(self.text_color) if isinstance(self.text_color, tuple) else (0, 0, 0),
                            'outline_color': tuple(self.outline_color) if isinstance(self.outline_color, tuple) else (255, 255, 255),
                            'outline_width_factor': int(getattr(self, 'outline_width_factor', 24) or 24),
                            'shadow_enabled': bool(self.shadow_enabled),
                            'shadow_color': tuple(self.shadow_color) if isinstance(self.shadow_color, tuple) else (0, 0, 0),
                            'shadow_offset': (int(getattr(self, 'shadow_offset_x', 2) or 2), int(getattr(self, 'shadow_offset_y', 2) or 2)),
                            'bg_style': str(getattr(self, 'text_bg_style', 'circle') or 'circle'),
                            'bg_opacity': int(getattr(self, 'text_bg_opacity', 130) or 0),
                            'bg_reduction': float(getattr(self, 'text_bg_reduction', 1.0) or 1.0),
                            'draw_bg': bool(draw_bg),
                        })
