...
For subtitle or synchronized-lyrics input, choose the file through **Input File(s)** as usual. The built-in **Subtitle Translation** prompt profile provides concise, natural dialogue or lyrics while retaining tone, character voice, meaningful line breaks, and protected formatting placeholders. Glossarion writes each translated `.srt`, `.ass`, or `.lrc` under its original filename inside the dedicated output folder (duplicate archive basenames receive `_2`, `_3`, and so on). For a subtitle ZIP, each file appears as soon as all API batches for that particular file are complete. If a model returns malformed structured output, omits a batch, or drops a protected formatting token, Glossarion does not create an incomplete final file.

You can also select a `.zip` containing one or more `.srt`, `.ass`, or `.lrc` files, including files located together beneath a wrapper subfolder. When translation or glossary processing starts, Glossarion safely inspects the archive and extracts every subtitle or lyrics member to a temporary folder. The members are exposed as chapters of one combined translation job, so **Batch mode** can translate different files concurrently up to the configured batch size. Other archive members are ignored. EPUB-formatted ZIPs continue through the EPUB workflow, while encrypted entries, unsafe paths, and archives beyond the extraction limits are rejected. All translated files from the archive are emitted into one subfolder named after the ZIP; duplicate basenames receive stable numeric suffixes. That shared folder also receives a `translation_progress.json` containing per-batch rows and a per-file summary. Selecting ZIP/SRT/ASS/LRC input clears a glossary that Glossarion automatically selected for the previous EPUB, but preserves a glossary explicitly loaded by the user. Glossarion does not create a new translated ZIP.

The **Extract Glossary** action supports the same subtitle formats and nested subtitle ZIPs. Glossarion sends only visible dialogue or lyrics for glossary analysis, excluding SRT cue numbers/timestamps, ASS headers/styles/drawing commands, LRC metadata/timestamps, and inline formatting tags. Each subtitle file becomes one glossary chapter, allowing Batch mode to process archive members concurrently, while the complete ZIP produces one shared archive-named glossary and progress file.
