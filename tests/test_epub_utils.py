...
def test_qa_scan_does_not_replace_protected_progress_issue(tmp_path):
    progress = {
        "chapters": {
            "86": {
                "actual_num": 86,
                ...
                "qa_issue_previews": {"PROHIBITED_CONTENT": "Provider blocked the response."},
                "duplicate_confidence": 0.25,
            }
        }
    }
    faulty_chapters = [{
        "filename": "chapter0086.xhtml",
        "chapter_num": 86,
        "issues": ["unwrapped_text_content", "word_count_mismatch"],
        "qa_issue_previews": {"unwrapped_text_content": "Blocked response body"},
    ...
    chapter = progress["chapters"]["86"]
    assert chapter["status"] == "qa_failed"
    assert chapter["qa_issues_found"] == ["PROHIBITED_CONTENT"]
    assert chapter["qa_issue_previews"] == {
        "PROHIBITED_CONTENT": "Provider blocked the response."
    }
    assert chapter["duplicate_confidence"] == 0.25


def test_qa_scan_does_not_replace_protected_issue_on_duplicate_entry(tmp_path):
    progress = {
        "chapters": {
            "86": {
                "actual_num": 86,
